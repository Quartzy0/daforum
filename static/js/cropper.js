...
(function(cropper, undefined) {
	...
	cropper.getCroppedInfo = function() {
		// work out the actual dimensions that need cropping
		let factor = currentDimens.factor;
		let x = Math.floor(overlay.x / factor);
		let y = Math.floor(overlay.y / factor);
		let width = Math.floor(overlay.width / factor);
		let height = Math.floor(overlay.height / factor);
